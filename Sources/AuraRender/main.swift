...
// ---- App Store Connect iPad 12.9" LANDSCAPE screenshots (exactly 2732×2048) ----
// Faithful to TodayView's wide (iPad/Mac) layout: the card column capped to 620pt and centred over a
// full-bleed AuraSky that carries the real WIDE landscape base (loaded off disk — the SwiftPM bundle has
// no asset catalog), with the same heroCarriesCondition/heroHorizon/heroAspect the app passes, so the sun
// sits behind the scenery instead of floating into the avisos band. Nature (landscape), never cityscape.
// Opaque, square corners (ASC rejects alpha), rendered at scale 2 off a 1366×1024 base for exact pixels.
@MainActor
...
@MainActor
func wideLandscapeBase(now: Date) -> Image? {
    let s = WeatherSnapshot.preview
    let isNight = (s.sunrise.map { now < $0 } ?? false) || (s.sunset.map { now > $0 } ?? false)
    return diskImage("hero_asset_creation/output/wide_landscape_\(isNight ? "night" : "day").png")
}
@MainActor
func appScreenASC(size: CGSize, now: Date) -> some View {
    ZStack(alignment: .top) {
        AuraSky(snapshot: .preview, now: now, heroImage: wideLandscapeBase(now: now),
                heroCarriesCondition: false,
                heroHorizon: HeroBackground.wideBaseHorizon(.landscape),
    ...
    .frame(width: size.width, height: size.height, alignment: .top)
    .background(Color.black)   // opaque: ASC rejects any alpha
    .environment(\.colorScheme, .dark)
    .fontDesign(.rounded)
}
// Two times for the two ASC slots: a cool bright afternoon (sun disc in frame, behind the cards) and a
// warm atardecer (the dusk sky at its most striking). Both land on the exact 2732×2048 landscape pixels.
for (label, when) in [("afternoon", todayAt(17, 0)), ("sunset", todayAt(20, 40))] {
    writeExact(appScreenASC(size: CGSize(width: 1366, height: 1024), now: when),
               name: "asc-ipad-nature-\(label)", size: CGSize(width: 1366, height: 1024), scale: 2)
}
