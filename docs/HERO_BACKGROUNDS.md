...
The Home Screen widgets (`AuraHomeSmall/Medium/Large` + the iPad `.systemExtraLarge`) draw the same live `AuraSky`, but they do **not** use the 48 portrait images — a short, wide slot wants a landscape crop. Instead they use a small set of **wide** day/night bases per family (`wide_landscape_day` / `wide_landscape_night`, `wide_city_day` / `wide_city_night`), resolved by `HeroBackground.wideBaseName(...)` and bottom-anchored so the horizon stays in frame.

Because the widget **gallery renders every supported family at once**, and `preparingThumbnail`/`Image` transiently decodes the full-resolution source (~7.7 MB for a 1600×1200 base) on each render, four simultaneous decodes on iPad (small + medium + large + extra-large) overrun WidgetKit's ~30 MB per-process budget — the losing previews get jetsammed and show a blank placeholder. So the widget loads **pre-sized tiers by name** via `AuraWidgets/WidgetHero.swift`: an extra-large tier at 1400 px (only the `.systemExtraLarge` family) and a lighter `_w` tier at 1000 px for every other family, both living in `AuraWidgets/Assets.xcassets`. No full-resolution decode ever happens in the extension. The widget also picks its family — landscape or cityscape — from the **Naturaleza / Ciudad scene** on `SelectHomeIntent` (`SceneStyle`), independent of the app's own Ajustes choice.

#### Why the widget doesn't match the app's hero, and how to add more images

The widget can't just reuse the app's hero art: the 48 portrait heroes are the wrong shape for a short, wide slot, and each one decodes to ~15 MB, so a single one nearly fills the widget's ~30 MB budget, and the gallery decodes up to four at once. That's the whole reason the widget stops at four wide bases (day/night × landscape/city) and lets `AuraSky` paint the hour and the weather on top, rather than tracking the hero image directly.

Adding a **few more** wide bases is safe for the budget — each widget still decodes exactly one base, so the four-at-once ceiling doesn't move. What it costs is new **wide** art from the `hero_asset_creation` pipeline (the widget can't crop the portrait heroes to wide). To get the widget background closer to the hero, generate golden-hour bases and, if wanted, condition variants, then:

1. **Make the art** as wide 1400×1050 scenes (same framing/sunless rule as the existing `wide_*` bases), for both families.
2. **Ship two tiers per image** in `AuraWidgets/Assets.xcassets`: the bare name at 1400 px and a `_w` variant at 1000 px (mirror the four bases already there). Keep the app-side full-res copies in `Aura/Assets.xcassets` only if the app's wide canvases want them too.
3. **Widen the resolver.** `HeroBackground.wideBaseName(_:isNight:)` currently keys only on day/night; extend it to take a time bucket (e.g. `dawn/day/dusk/night`) and, optionally, condition, returning names like `wide_landscape_dawn` / `wide_city_overcast_day`. `WidgetHero.base(...)` already appends `_w` and falls back `sized → name → nil`, so any bucket without art degrades to the nearest shipped base and finally to the procedural sky, the same incremental, no-cost-until-shipped contract the 48-grid has.

Suggested first increment (8 images, 16 with tiers): `wide_{landscape,city}_{dawn,dusk}` — the golden hours the current day/night pair misses most. Deferred until the art exists; no code has been changed for it yet.
