...
Aura has no backend. The iPhone app is the fetch hub: it calls AEMET, normalizes each reading into a small `WeatherSnapshot`, and writes it to a shared App Group cache that the widgets read directly and the Watch receives over WatchConnectivity. Nothing else talks to AEMET, so one fetch feeds every surface.

That shape is forced by AEMET's OpenData limits:

- **50 requests per minute, global.** Paced on-device by a request pacer with 429 backoff.
- **Two calls per reading.** Every value is an envelope call that returns a temporary `datos` URL, then a fetch of that URL.
- **Different cadences.** Forecasts refresh a few times a day, observations roughly hourly. Aura refreshes on that real cadence rather than on every widget wake, and skips a location it fetched recently.
- **The key expires every 3 months.** It is free and tied to an email. When it stops working, request another the same way. The key lives in the Keychain, never in the binary or the repo.

AEMET forecasts are keyed by INE municipality code and observations by station code, so the app bundles a lookup that turns a location into its nearest municipality and its nearest station. When the network is unavailable, or the Watch is away from the phone, every surface shows the last cached values. That offline fallback is core to the design, not an error state. Sun and moon times are computed on-device (NOAA / Meeus solar equations, matching the Observatorio Astronómico Nacional tables), with no AEMET call.
...
Building the `Aura` scheme also builds and embeds the Watch app + complication (each target keeps its own SDK, so don't pass `-sdk`, which would force one SDK onto every target). Run the `AuraWatch` scheme to install straight to a paired watch.
